...
RSpec.describe Sanitizers::BaseSanitizer do
  ...
  describe "#extract_inner_html" do
    it "calls InnerHtmlExtractor with the correct arguments and returns the result" do
      ...
      allow(Sanitizers::Utilities::InnerHtmlExtractor).to receive(:new).with(html_content:, element: "body").and_return(extractor_double)
      allow(extractor_double).to receive(:extract_inner_html).and_return("<body></body>")

      result = sanitizer.send(:extract_inner_html, html_content:, element: "body")

      expect(result).to eq("<body></body>")
      expect(Sanitizers::Utilities::InnerHtmlExtractor).to have_received(:new).with(html_content:, element: "body")
  ...
  describe "#remove_elements" do
    it "calls ElementRemover with the correct arguments and returns the result" do
      ...
      allow(Sanitizers::Utilities::ElementRemover).to receive(:new).with(html_content:).and_return(remover_double)
      allow(remover_double).to receive(:remove_elements).and_return("<html><body></body></html>")

      result = sanitizer.send(:remove_elements, html_content:)

      expect(result).to eq("<html><body></body></html>")
      expect(Sanitizers::Utilities::ElementRemover).to have_received(:new).with(html_content:)
  ...
  describe "#beautify" do
    it "calls Beautifier with the correct arguments and returns the result" do
      ...
      allow(Sanitizers::Utilities::Beautifier).to receive(:new).with(html_content:).and_return(beautifier_double)
      allow(beautifier_double).to receive(:beautify).and_return("<html>\n  <head>\n  </head>\n  <body>\n  </body>\n</html>")

      result = sanitizer.send(:beautify, html_content:)

      expect(result).to eq("<html>\n  <head>\n  </head>\n  <body>\n  </body>\n</html>")
      expect(Sanitizers::Utilities::Beautifier).to have_received(:new).with(html_content:)
